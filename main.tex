...
\title{Algorithmic Foundations of Collective Decision Making}
\author{Drew D. Kavi}
...
These revision notes were made to supplement the lectures.

A course on social choice theory --- 
a study of how individual preferences can be combined, 
...
\begin{definition}
    ...
    \begin{itemize}
        \item Reflexive: if $\forall x \in U$, $x R x$.
...
\begin{definition}[Strict Ranking]
    ...
    $U$ that is reflexive, complete, transitive and antisymmetric.
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        The $\impliedby$ direction is immediate. For $\implies$ take 
...
\begin{definition}[Prefernces over Alternatives]
    ...
    A \emph{prefernce relation} on $U$ is linear order on $U$.
...
\begin{definition}[Preference Profile]
    ...
    \footnote{We use $N$ as the superscript (instead of $n$) because we think of $R_N$ as a function from $N \mapsto \mathcal{L}(U)$.}.
\end{definition}
...
\begin{definition}[Basic Fairness Conditions]
    ...
    \begin{itemize}
        ...
        Formally, for all pairs $R_N \in \mathcal{L}(U)^N$, $R'_N \in \mathcal{L}{U'}$ such that there is a bijection
...
\begin{definition}[Pareto Optimality]
    ...
    \begin{itemize}
        \item $x$ \emph{Pareto-dominates} $y$ if $\forall i \in N$, $x \succ_i y$.
    ...
    to see as such, 
    pick an alternative,
    if it is Pareto-optimal, we are done, otherwise there is an alternative that strictly Pareto-dominates it,
    consider this alternative, if it is Pareto-optimal, we are done, otherwise there is an alternative that strictly Pareto-dominates it.
    Given that $U$ is finite, all that remains is to show that we cannot cycle here, 
    but this is clear since the strict Pareto-relation is transitive (and irreflexive).
...
\begin{definition}[Monotonicty]
    ...
    for all $R_N, R_N' \in \mathcal{L}(U)^N$ and $A \in \mathcal{F}(U)$,
    such that the same conditions hold, and where $R_N \neq R_N'$, 
...
\begin{definition}[Independence of Infeasible Alternatives (IIA)]
    ...
    if, 
    for all $A$, $R_N, R_N'$:
    $R_N |_A = R_N' |_A$ implies $f(R_N, A) = f(R_N', A)$.
\end{definition}
...
\begin{theorem}[Arrow's Impossibility Theorem]
    ...
    and transitive rationalisability is a \emph{dictatorship}
    \footnote{Where by a dictatorship we mean that the winner is always the top ranked alternative of a specific voter.}.
\end{theorem}
...
\begin{theorem}[Arrow's for SWFs]
    ...
    for three or more alternatives (and two or more voters which we now assume
    from here on out).
\end{theorem}
...
\begin{remark}
    ...
    and so if we want these seemingly easy and desirable properties
    it seems we must give up.
    We spend much of the course finding ways to escape from this 
    impossibility:
    \begin{itemize}
...
\begin{remark}
    ...
    is the assumption that all preference profiles are possible,
    ...
    which allows for better axiomatic results.
\end{remark}
\begin{example}
...
\begin{remark}
    ...
    In particular, 
    if $\gtrsim_M$ is transitive
    then $\gtrsim_M \in \mathcal{R}(U)$, thus 
...
\begin{remark}
    ...
    in $R_N$ and $R_N'$ must be the same when restricted to $A$ (so IIA is satisfied).
...
\begin{definition}
    ...
    This gives a kind of monotonicity condition which is the intuitive notion of single-peakedness.
...
theorem, which follows(-ish) from the previous section.
...
\end{intuition}
\begin{proof}[Sketch]
    \unclear{See lectures for a sketch of the proof.}
\end{proof}
...
\begin{remark}[Median Voting]
    ...
    We see that in $\mathcal{D}_{\text{SP}}^{\trianglelefteq}(U)$,
...
\begin{idea}
    ...
    Suppose that $x$ is a bottom ranked alternative in
...
\begin{definition}[Scoring Rule]
    ...
    A \emph{scoring rule} is a SCF that chooses those 
...
\begin{example}
    ...
    \unclear{Think why? Also, does Borda satisfy it?}
\end{example}
...
\begin{idea}[Unifying Borda and Condorcet]
    ...
    \begin{itemize}
        ...
        one exist, otherwise the Borda winner(s) (by definition a Condorcet extension)
...
\begin{proof}
    ...
    some alternative and the profile is symmettric in 
...
\begin{idea}[Social Preferences]
    ...
    where $\mathcal{F}(\mathcal{L}(U))$ is the set of non-empty subsets of $\mathcal{L}(U)$.
...
\begin{example}
    ...
    edge weight in the margin graph.
...
\end{theorem}
\begin{proof}
    \unclear{See lectures for proof.}
\end{proof}
...
\begin{example}
    ...
    it is not monotonic, nor a \emph{Condorcet extension}.
\end{example}
...
\begin{definition}[PUT Ranked Pairs]
    ...
    (but annoyingly, does not satisfy independence of clones).
...
\begin{remark}
    ...
    the Gore, Bush, Nader case (2000 US Presidential Election).
\end{remark}
...
\begin{definition}
    ...
    it strictly wins in the majority relation, and 
...
\begin{remark}
    ...
    \begin{enumerate}
        ...
        $a$ defeats $b$ by checking if $a \succ_M b$ and $\operatorname{strength}(b, a) < m(a, b)$: $O(|A|^2)$.
...
\begin{proof}[Sketch]
    ...
    Then, Lemma~\ref{l:strength-characterization-SC} gives that 
    $b \succ_M a$ and $\operatorname{strength}(b, a) < m(a, b)$.
...
\begin{idea}
    What does multiwinner voting mean? So far, 
...
\begin{definition}
    ...
    For each party $P_i$ let $v_i$ denote the number of votes it receives.
...
\begin{example}[The Population Paradox]
    A party that loses votes may gain seats at the expense of a party that gains votes.
    Formally, 
...
\begin{definition}[Approval-Based Multiwinner Rule]
    ...
    $C$, most often we denote it $W \subseteq C$.
...
\begin{definition}[Satisfaction-AV (SAV)]
    ...
    chooses committee(s) $W$ that maximize the total satisfaction:
    \[
...
\begin{definition}[Greedy-AV]
    ...
    $A_i \cap W \neq \emptyset$.
    If all voters are represented, we can add any candidates we like to fill up the committee.
\end{definition}
...
\begin{remark}
    ...
    Can we make a similar comment about 
...
\begin{example}\label{e:fail-JR}
    ...
    many approval-based multiwinner rules violate JR.
    \begin{itemize}
...
\begin{remark}[Finding Proportional Committees Efficiently]
    ...
    \begin{itemize}
        \item A committee satisfying JR can be found efficiently (Exercise: Why?)
        \item PAV satisfies EJR$^+$ (and EJR) but as noted, cannot be computed efficiently
    \end{itemize}
\end{remark}
...
\end{theorem}
\begin{proof}[Sketch]
    Start with an arbitrary committee of appropriate size.
...
\begin{remark}
    For several reasons, this doesn't make for a particularly convincing or 
